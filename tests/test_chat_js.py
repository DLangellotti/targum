...
def test_an_offer_to_look_wider_is_a_card_and_the_press_asks_again() -> None:
    """The words that would be searched are on the card, so a reader reads the question
    before agreeing to it. The press posts the same line again with `wider` set."""
    offer = {"asked": "מתכונים בעברית", "why": "I asked for recipes and got news.", "sites": 42}
    page = run(
        do=[
            {"type": "say", "text": "find me recipes"},
            {"type": "stream", "event": "wider", "data": json.dumps(offer, ensure_ascii=False)},
            {
                "type": "stream",
                "event": "done",
                "data": json.dumps({"text": "Not what you asked."}),
            },
            {"type": "press", "selector": "quote-go"},
        ],
        answers={"/chat/say": {"chat": "abc", "turn": 1}},
    ...
    card = page["cards"][0]
    assert card["title"] == "מתכונים בעברית", "the search is shown, not described"
    assert card["meta"] == "targum looks at 42 Hebrew sites. This asks the whole web."
    assert card["button"] == "Look wider"
    assert card["note"] == "I asked for recipes and got news."
    assert "$" not in json.dumps(card), "never money"
    posted = [p for p in page["posted"] if p["path"] == "/chat/say"]
    assert len(posted) == 2, "the press asks again"
    assert posted[0]["body"].get("wider") is None, "the first line was not a press"
    assert posted[0]["body"]["text"] == "find me recipes"
    assert posted[1]["body"]["wider"] is True and posted[1]["body"]["text"] == "מתכונים בעברית"


def test_an_offer_with_no_words_draws_nothing() -> None:
    page = run(
        do=[
            {"type": "say", "text": "find me recipes"},
            {"type": "stream", "event": "wider", "data": json.dumps({"sites": 42})},
        ],
        answers={"/chat/say": {"chat": "abc", "turn": 1}},
    )
    assert page["cards"] == []
