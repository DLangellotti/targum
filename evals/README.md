...
| `recast` | `judge_ok_share`, `lemma_overlap`, `unpaired`, the chat's `> ` line against a person's rendering of the same English: `corpus=tatoeba`, volunteers' sentences, or `corpus=flores-plus`, FLORES+'s professional renderings (`--reference flores`) | `scripts/eval_recast.py` (targum-internal#219, #221) |
